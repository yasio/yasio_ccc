...
public:
  explicit file_writer(v8::FunctionCallbackInfo<v8::Value> const& args)
  {
    if (args.Length() == 1)
    {
      //v8::String::Utf8Value str(args[0].As<v8::Local<Value>());
      //open(*str);
    }
  }
...
public:
  explicit file_reader(char const* path) { open(path); }
...
void test()
{
  ...
  auto jsGlobal = context->Global();
  //v8pp::module addon(v8::Isolate::GetCurrent());

  // set bindings...
  // addon.function("fun", &function).class_("cls", my_class);

  // set bindings as exports object prototype
  // auto global = obj->_getJSObject();

  // global->SetPrototype(addon.new_instance());
  ...
  file_base_class.function("close", &file_base::close)
  ...
  file_writer_class.ctor<v8::FunctionCallbackInfo<v8::Value> const&>()
      .inherit<file_base>()
  ...
  file_reader_class.ctor<char const*>().inherit<file_base>().function("open", &file_reader::open).function("getln", &file_reader::getline);

  // Create a module to add classes and functions to and return a
  // new instance of the module to be embedded into the v8 context
  v8pp::module m(isolate);
  m.function("rename", &rename).class_("writer", file_writer_class).class_("reader", file_reader_class);

  // return scope.Escape(m.new_instance());

  // set bindings in global object as `mylib`
  jsGlobal->Set(context, v8::String::NewFromUtf8(isolate, "v8pptest").ToLocalChecked(), m.new_instance());
}
